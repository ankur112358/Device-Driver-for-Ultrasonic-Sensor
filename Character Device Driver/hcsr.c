	...
	if (irq == gpio_to_irq(hcsr_obj->pin.gpio_echo_pin))
	{
		if (check ==0)
		{
            //printk("gpio pin is low\n");
	 		tsc_falling=get_tsc();                                                              /////after getting the timestamp for rising edge, this will get the time stamp for failling edge
...
/////once the job is done this function will free the gpio pins
...
/////when called upon this function will start sampling by sending the trigger pulse, stores the distance and its timestamp in fifo buffer
...
////init function called upon insmod, used for registering misc devices
static int __init init_gpio_module(void){

    int i, error;
    char *name;
    struct hcsr_dev* device_pointer, *temp=NULL;
    HEAD = NULL;
	...
	error = misc_register(&device_pointer->device);         //////registering misc device
		if(error){
